...
function StudyView({ deckId, queue, queueIndex, mode, progress, onBack, onShuffle, onNext, onGrade, onRecordAgain, onSaveTime }: { deckId: DeckId; queue: CaseData[]; queueIndex: number; mode: SessionMode; progress: ProgressStore; onBack: () => void; onShuffle: () => void; onNext: () => void; onGrade: (rating: Rating, elapsed?: number) => void; onRecordAgain: () => void; onSaveTime: (elapsed: number) => void }) {
  ...
  const revealAsAgain = () => { setAnswerShown(true); setAutoRated(true); onRecordAgain(); };
...
export default function Home() {
  ...
  const recordRating = useCallback((rating: Rating, elapsed?: number) => { const current = currentRef.current; if (!current.item) return; const key = caseKey(current.deckId, current.item); persist({ ...progress, [key]: nextProgress(progress[key], rating, Date.now(), elapsed) }); }, [persist, progress]);
  const grade = useCallback((rating: Rating, elapsed?: number) => { recordRating(rating, elapsed); window.setTimeout(advance, 0); }, [advance, recordRating]);
  ...
  if (view === 'study') return <StudyView key={`${deckId}:${queueIndex}`} deckId={deckId} queue={queue} queueIndex={queueIndex} mode={mode} progress={progress} onBack={() => setView('dashboard')} onShuffle={() => startStudy(deckId, 'shuffle')} onNext={nextCard} onGrade={grade} onRecordAgain={() => recordRating('again')} onSaveTime={saveTime} />;
